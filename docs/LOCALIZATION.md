...
New user-facing T24–T29 strings use stable semantic keys rather than hard-coded text. T24B adds complete key families for `Administration.*`, `Overview.*`, `Devices.*`, and `Diagnostics.*`; exact `pt-BR`/`en-US` parity is tested. Planned T25+ examples include `Config.Ups.Driver`, `Config.Ups.Port`, `Config.Ups.Protocol`, `Config.Ups.RuntimeCalibration`, `Config.Review.Apply`, and `Config.Review.Discard`. Keys are never Portuguese display text.
