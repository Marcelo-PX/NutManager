...
New user-facing T24–T29 strings use stable semantic keys rather than hard-coded text. T24B adds complete key families for `Administration.*`, `Overview.*`, `Devices.*`, and `Diagnostics.*`. T25 adds the generic `Semantic.*` families. T26 adds the `Ups.*` graphical configuration families. T27 adds `Config.General.*`, `Config.Server.*`, `Config.Listen.*`, `Config.Tls.*`, and production descriptor label/help keys under `Config.Field.*`; every `nut.conf`/`upsd.conf` descriptor has a label and help entry in both official cultures. Exact `pt-BR`/`en-US` key parity is tested. Core descriptors carry only resource keys; keys are never Portuguese display text.
