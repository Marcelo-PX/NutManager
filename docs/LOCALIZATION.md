...
New user-facing T24–T29 strings use stable semantic keys rather than hard-coded text. T24B adds complete key families for `Administration.*`, `Overview.*`, `Devices.*`, and `Diagnostics.*`. T25 adds `Semantic.Review.*`, `Semantic.Validation.*`, `Semantic.Custom.*`, `Semantic.State.*`, `Semantic.Operation.*`, `Semantic.Sensitive.*`, `Semantic.Activation.*`, and representative `Semantic.Field.*` labels. Exact `pt-BR`/`en-US` key parity is tested. Core descriptors carry only resource keys; keys are never Portuguese display text.
