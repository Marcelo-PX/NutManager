...
T25 supplies the shared framework and representative descriptors. T26 implements the production driver-aware `ups.conf` form and documented `runtimecal` configuration assistant. Its schemas carry presentation metadata, invariant codecs, applicability, bounds where the manual defines them, and change-only sensitivity. Unknown or newly installed drivers remain explicit limited-validation inputs; changing driver context marks incompatible fields unsupported and never deletes them automatically. T27 owns final `nut.conf`/`upsd.conf` forms; T28 owns final `upsd.users`/`upsmon.conf` and credential UX; T29 owns final graphical-configuration hardening. The T15 entry model remains available as a compatibility projection, while `ups.conf` mutations originate in the semantic draft.
