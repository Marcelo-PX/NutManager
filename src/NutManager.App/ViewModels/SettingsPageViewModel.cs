...
public sealed partial class SettingsPageViewModel : PageViewModel
{
    ...
    public bool IsProfileDraftDirty => _isCreatingProfile || (_draftSourceId is { } id && ManagedProfiles.FirstOrDefault(profile => profile.Id == id) is { } profile && !ProfileDraft.Matches(profile));
    ...
    private async Task SaveProfileAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (store is null || !CanPersistProfiles)
        {
        ...
        try
        {
            ...
            var profiles = _confirmedProfiles.Profiles
                .Where(profile => profile.Id != updated.Id)
                .Append(updated)
                .OrderBy(profile => profile.Name, StringComparer.OrdinalIgnoreCase)
                .ToArray();
            var activeId = _confirmedProfiles.ActiveProfileId;
            var document = new ManagedNutServerProfiles(_confirmedProfiles.SchemaVersion, activeId, profiles);
    ...
    {
        var store = _profileStore;
        if (SelectedManagedProfile is null || store is null || !CanPersistProfiles)
        {
            return;
        }
        ...
        try
        {
            var document = new ManagedNutServerProfiles(
                ...
                _confirmedProfiles.Profiles.Where(profile => profile.Id != SelectedManagedProfile.Id).ToArray());
            await store.SaveAsync(document, cancellationToken);
    ...
    {
        var store = _profileStore;
        if (SelectedManagedProfile is null || store is null || !CanPersistProfiles || SelectedManagedProfile.Id == _confirmedProfiles.ActiveProfileId)
        {
            return;
        }
        ...
        try
        {
            var document = new ManagedNutServerProfiles(_confirmedProfiles.SchemaVersion, SelectedManagedProfile.Id, _confirmedProfiles.Profiles);
            await store.SaveAsync(document, cancellationToken);
