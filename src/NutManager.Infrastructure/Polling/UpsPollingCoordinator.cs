...
public sealed class UpsPollingCoordinator : IUpsPollingCoordinator
{
    ...
    private CancellationTokenSource? _session; private Task? _loop; private readonly SemaphoreSlim _refreshSignal = new(0, 1); private int _generation; private bool _disposed;
    ...
    {
        ThrowIfDisposed(); _session?.Cancel(); var previous = _loop; ++_generation;
        if (previous is not null) await ObserveAsync(previous);
        _session?.Dispose(); var source = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken); _session = source; var generation = _generation;
    ...
    public Task RefreshAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_loop is null || State.UpsName is null) return Task.CompletedTask;
    ...
    private async Task RunAsync(string upsName, int generation, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            try
            {
                var delay = Task.Delay(_interval, _timeProvider, token);
                var refresh = _refreshSignal.WaitAsync(token);
    ...
    public void Dispose() { if (_disposed) return; _disposed = true; _session?.Cancel(); _session?.Dispose(); _refreshSignal.Dispose(); }
